...
const texts = ['Loading Assets…', 'Preparing Interface…', 'Launching Experience…']
...
export default function Preloader({ onFinish }: PreloaderProps) {
  ...
  useEffect(() => {
    if (step < texts.length - 1) {
      const timer = setTimeout(() => setStep((prev) => prev + 1), DISPLAY_DURATION)
      return () => clearTimeout(timer)
    } else {
      const finishDelay = setTimeout(() => setTextsFinished(true), DISPLAY_DURATION)
